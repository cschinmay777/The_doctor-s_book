...
class StatisticsController extends GetxController {
  ...
  Future getStatistics() async {
    var list = await FirebaseFirestore.instance
        ...
        .get();
    if (list.data() != null) {
      var records = list.data()!['patients'].map<Patient>((e) => Patient.fromJson(e)).toList();
          ...
          for (var symptom in entry.symptoms!) {
            if(statisticsData.containsKey(symptom)){
              if(statisticsData[symptom]!.containsKey(entry.date.month)){
                statisticsData[symptom]![entry.date.month] = statisticsData[symptom]![entry.date.month]! + 1;
              }
              else{
